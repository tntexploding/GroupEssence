...
OCR。AstrBot 插件不加载 OCR 或独立 HTTP 依赖，查询图片下载仅使用标准库。
...
`/精华最近` 和 `/精华查询` 在文字摘要之后发送对应的图片消息，并标注记录序号。
云端统一命令 `/ge recent`、`/ge search` 复用同一实现。权限与目标群选择不变。
`max_reply_images` 默认 5，范围 0–10，设为 0 恢复纯文字回复；
`enable_image_enrichment` 仍是离线 OCR 预留项，应保持关闭，图片回传不依赖它。

每张图片最多 5 MiB，单次查询共用 20 秒下载预算。仅接受明确允许的 QQ 精华/CDN
HTTPS 来源，校验并固定公网解析地址，重定向重新检查；不读取任意 URL 或本地路径。
图片以字节发送，AstrBot 与 NapCat 无需共享图片目录，也不新增端口或 Token。
缓存位于插件数据目录 `reply_images/<群号>/`，按需创建，总量最多 128 MiB / 256 个文件，
按最近使用顺序淘汰；此缓存不是永久图片归档，也不会改变消息数据库或其备份策略。
图片过期、下载失败、超限时保留文字并提示原因；可先同步刷新引用，再缩小范围查询。
纯文字和只读验收不会下载图片；此功能不执行 OCR、不理解图片内容、不调用 LLM。
