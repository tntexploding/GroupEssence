...
> 进入时间索引，插件仍不监听全部群消息。
>
> 0.4.0 P0 修订：失败详情现改为持久化的到期重试，而非永久跳过；计划同步、状态转换
> 告警、在线备份和脱敏健康快照已经实现，但全部按远端部署指南默认关闭并分阶段灰度。
> 本文前半部分保留最初薄适配设计背景，当前运行约束以架构说明和部署指南为准。
...
3. 按远端部署指南只为一个白名单群灰度计划同步与在线备份；
4. 再决定是否增加白名单群；图片 OCR 与普通成员查询继续单独评估。
